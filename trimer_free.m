%---------- find atoms who form trimer interactions- free _--------------
clear all
tic
cutoff1=7.5;
cutoff=6;
...
distance1=[];
   ...
   a_bjurm_dist = find(distance_min1(2,:)<cutoff);
   a_bjurm = (a22_free(a_bjurm_dist));
   a_31 = (a231(a_bjurm_dist));
   a_32 = (a232(distance_min(1,a_bjurm_dist)));
...
distance3=[];
   for iii=1:length(a_bj)
       ...
       distance_min2(1,iii)=find(distance2 == min(distance2));
       distance_min2(2,iii) = distance2(distance_min2(1,iii));
       else
           k_cl=a_bj(iii,2); % do for CL
       x_ion=a1(k_cl,3);
       y_ion=a1(k_cl,4);
       z_ion=a1(k_cl,5);
       ...
       for jjj=1:length(a_rest)
           ...
           y_water=a1(w_rest,4);
           z_water=a1(w_rest,5);
           distance3(jjj)=sqrt((x_ion-x_water)^2+(y_ion-y_water)^2+(z_ion-z_water)^2);
       end
       if min(distance3)<cutoff
       distance_min2(1,iii)=find(distance3 == min(distance3));
       distance_min2(2,iii) = distance3(distance_min2(1,iii));
       end
       end
   ...
   a22_free_dist1 =[];
   a22_tri=[];
   a22_free_dist1 = find(0<distance_min2(2,:));
